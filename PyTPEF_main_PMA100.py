...
class Measure(QObject):
    ...
    def measure_step_3(self, wavelength, sample, i, angle, all_power_values, y_dark, power_dark_mean,
        ...
        self.send_data_to_user.emit(f"Corrected fluorescence is {round(corr_dye_area,2)}.")
        full_corrected_area = rdf()._Emission_Correction(corr_dye_area, intensity_dark_corr,
                                                         spectro_wavelength, self.ccd_correction_factor, emission_filter=self.current_emission_filter, sample_name=sample, power=power_dark_corr, wavelength=wavelength)
...
class MyApp(QWidget):
    ...
    def run(self, intensity_dark):
        ...
        x, y , raw_fluo= self.spectro.Maya(int(self.integration_time_value))._data_read_scan(number_of_scan=self.scan_value, intensity_dark=intensity_dark)
    ...
    def plot_s2_from_signal(self, simulation):
        ...
        try :
            ...
            sample = self.select_sample()
            wavelength_t = self.select_wavelength()
            self.tpef_calculation_for_single_measure(sample=sample, wavelength=wavelength_t, simulation=False)
        ...
        for selected_wavelength in self.ls_done_wavelengths.selectedItems():
            ...
            for i in range(number_of_measure):
                try:
                    ...
                    square_power = round(square_power, 2)
