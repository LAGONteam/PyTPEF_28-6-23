...
EMISSION_FILTER={"FES_750":(542.0,684.0), "FES_800":(397.0,799.0), "FES_700":(433.0,695.0), "E750SP-2P":(509.9, 750.1), "E650SP-2P":(431.0,760.0), "E650":(480.5, 625.0), "FF01-650SP25":(429.9,630.1)} #"E650":(480.5, 625.0)
...
class Read_Data_File():
    ...
    def _Emission_Correction(self, corr_dye_area, table_fluo, table_wavelength, corr_fact, emission_filter, sample_name,power, wavelength):
        ...
        wavelength_limited = table_wavelength[i:z]
        ...
        if a == b:
            ...
            # self._Figure_Duo(x1=wavelength_limited, y1=emission_limited_ccd_corr, x2=table_wavelength, y2=table_fluo, name=f"Spectres emission corrigés CCD corr & etha pour {sample_name} at {wavelength} nm and {power} mW")
    ...
    def _Quad_Log(self, list_of_fully_corrected_area, list_of_square_dark_corr_power):
        ...
        try:
            ...
            p_max= np.amax(list_of_square_dark_corr_power)
            p_min=np.amin(list_of_square_dark_corr_power)
            table_data = np.linspace(start= p_min, stop= p_max, num=10)
            fluo_data = []
            for power_value in table_data:
                temp = power_value*area
                fluo_data.append(temp)
            fluo_data=np.array(fluo_data)
            correlation_coeff=round(reg_lin[2]*reg_lin[2],5)
            self._Figure_Duo(x1=list_of_square_dark_corr_power, y1=list_of_fully_corrected_area, x2=table_data, y2=fluo_data, name=f"Quad_{area} r²= {correlation_coeff}")
            if correlation_coeff <=1:
                p_temp= np.array(list_of_square_dark_corr_power)
                f_temp = np.array(list_of_fully_corrected_area)
                fpp_temp = f_temp/p_temp
                area = np.mean(fpp_temp)
                print(p_temp, type(p_temp))
                print(f_temp, type(f_temp))
                print(fpp_temp, type(fpp_temp))
